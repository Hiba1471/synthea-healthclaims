...
| `q3_top_hospitals_and_conditions_2020_2024.csv` | 50 | The named entities behind those curves, the individual top patients, conditions and organisations. |
...
| `q1_pregnancy_sensitivity_2020_2024.csv` | 2 | **Sensitivity test: does the condition ranking survive correcting pregnancy's 8.6x pricing error?** No, for the ranking. Repricing every pregnancy claim down by 8.6x drops it from rank 1 (39.8%) to rank 5 (7.1%); allergy and gingivitis move to 1st and 2nd. Top-5 share of diagnosed spend falls 74.1% -> 60.1%, top-20 falls 90.8% -> 85.8%. Patient-level spend concentration (Finding 4) goes the OTHER way in this single-condition test: repricing pregnancy alone down makes the top 1% of patients carry MORE of total spend (11.8% -> 15.4%). **This does not generalise**, see `q1_multi_condition_sensitivity_concentration_2020_2024.csv`, where correcting more than pregnancy reverses it. |
| `q1_multi_condition_sensitivity_ranking_2020_2024.csv` | 20 | **The condition ranking under a broader correction: eleven of the top twenty conditions repriced together, not just pregnancy.** The ranking reshuffles far more than the pregnancy-only test suggested. Pregnancy falls to 2nd (14.7%). Chronic kidney disease, never touched, because it was checked separately and found NOT inflated, rises to 1st (19.2%) purely because everything around it shrank. Allergy drops out of the top twenty entirely. Top-5 share of diagnosed spend falls to 47.2%, top-20 to 78.3% (from 90.8% as billed). Five conditions with a similarly large gap (cardiac imaging findings, small cell lung cancer, child ADHD, ischemic heart disease, drug abuse) were deliberately left uncorrected: no fixed visit cadence exists to anchor a defensible correction factor on them, unlike dialysis's known three-times-weekly schedule. |
| `q1_multi_condition_sensitivity_concentration_2020_2024.csv` | 3 | **The reversal that matters most: patient-level concentration under the broader correction comes back down, not up.** The pregnancy-only test found the top 1% of patients carrying MORE of total spend once repriced (11.8% -> 15.4%). Correcting all eleven confidently-inflated conditions together brings that back down to 12.5%, close to the as-billed figure, because several of the other inflated conditions (allergy, dental, colon polyp, bronchitis, UTI) are themselves broadly spread across many patients, so correcting them pulls weight back away from the very top. **The direction of this specific effect depends on how many conditions are corrected and is not settled by either test alone;** that spend concentrates in a minority of patients at all is the more durable part of Finding 4. |
| `q1_scenario_comparison_2020_2024.csv` | 5 | **Five conditions' share of diagnosed spend under all three pricing scenarios in one place**, backing the supplementary sensitivity chart in the report. Pregnancy 39.81% as billed / 7.15% pregnancy-only-repriced / 14.74% all-eleven-repriced. Allergy 11.88 / 18.33 / 0.42, it becomes the largest condition when only pregnancy is corrected, then drops out of the top twenty entirely once its own ~89x gap is corrected too. Kidney disease 6.04 / 9.32 / 19.21, rising to 1st without ever being corrected itself. |
| `q1_robustness_three_findings_2020_2024.csv` | 2 + 12 | **Robustness test of three findings against supplied price-adjustment factors.** Pregnancy's share of diagnosed spend 39.81% -> 14.76% ($28.93B -> $3.36B). Member share of all billed 20.44% -> **23.25%**, the share RISES while the dollars FALL ($20.26B -> $11.43B), because the 11 adjusted conditions carry $54.89B at a 17.73% member share against $44.22B at 23.80% for everything else, so shrinking the low-share bucket reweights the blend upward. Commercial:government ratios are **exactly unchanged** for every condition tested (pregnancy 25.2x, gingivitis 17.1x, allergy 9.4x before and after), scaling billed and paid by the same factor cancels in a ratio, so the benefit-design finding is immune to this class of defect. |
| `q1_condition_breakdown_repriced_2020_2024.csv` | 185 | **Reference table, not a corrected dataset, nothing in the report is drawn from it.** All 185 conditions with the eleven confidently-inflated ones divided down to their real-world benchmark and the other 174 left as billed. Written for a plan to replace the report's primary Q1 charts that was then cancelled; kept because the question is worth having answered and traceable. Corrected diagnosed total is $22.9B against $72.7B as billed; kidney disease leads at 19.2%, pregnancy 2nd at 14.7%. Pregnancy's corrected cost per person lands at $18,864 against the $18,865 benchmark used to derive its divisor, which is the arithmetic closing on itself, not independent confirmation. |
| `q2_care_type_breakdown_repriced_2020_2024.csv` | 15 | **The one genuinely new result from the cancelled repricing work: correcting prices moves member-paid SHARE, and not all in one direction.** Allergy & immune rises 10.0% -> 15.0%, cancer falls 8.3% -> 7.4%, kidney & urinary 14.2% -> 13.0%, dental 27.9% -> 25.8%. That is the annual out-of-pocket cap doing what `q2_commercial_cap_by_care_type.sql` already documented: a large bill blows past the cap so the member's share of it is small, while a smaller bill stays under it and the member carries proportionally more. Correcting inflated prices downward pushes several categories back under the cap. Every category containing a corrected condition moved; every category containing none is unchanged to the decimal, which is the check that the query does what it claims. |
| `q2_oop_percentiles_repriced_2020_2024.csv` | 15 | **Maternity's out-of-pocket dollar figure, cited repeatedly in this project as the clearest example of real member exposure, is mostly the pricing defect.** Median falls $10,765 -> $1,308, P90 $64,681 -> $7,630, once its price is corrected. Dental & oral also moves ($1,358 -> $144). Every other category is unchanged, containing no corrected condition. Infections (other) becomes the highest median at $1,355, itself untouched -- ahead of maternity by only $47. |
...
| `q3_visit_frequency_vs_case_mix_2020_2024.csv` | 119 | **The file that eliminated Q3's last candidate lever.** Spread in procedures per claim between hospitals, per condition. The typical condition is **0.027**, sites do effectively identical amounts of work. Only 12 of 119 exceed 0.5 and all but one are small; the exception is normal pregnancy at 0.962 across 856 sites and $28.9B. Its query header carries the second test, which shows even that is pathway split (cheap sites deliver babies, dear sites run antenatal clinics) rather than practice variation, and records the $17.3B "excess over median" figure as a trap not to be quoted. |
